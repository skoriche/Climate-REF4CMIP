...
Revision ID: 6a8cd12f702e
Revises:
Create Date: 2025-05-01 17:29:03.996026
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("diagnostic_id", "dataset_key", name="execution_group_ident"),
    )
    with op.batch_alter_table("execution_group", schema=None) as batch_op:
        batch_op.create_index(batch_op.f("ix_execution_group_dataset_key"), ["dataset_key"], unique=False)
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), server_default=sa.text("(CURRENT_TIMESTAMP)"), nullable=False),
        sa.Column("updated_at", sa.DateTime(), server_default=sa.text("(CURRENT_TIMESTAMP)"), nullable=False),
        sa.ForeignKeyConstraint(["execution_group_id"], ["execution_group.id"], name="fk_execution_group_id"),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["dataset_id"], ["dataset.id"], name=op.f("fk_execution_result_dataset_dataset_id_dataset")
        ),
        sa.ForeignKeyConstraint(
            ["execution_result_id"],
            ["execution.id"],
            name=op.f("fk_execution_result_dataset_execution_result_id_execution"),
        ),
    )
    op.create_table(
        "metric_value",
    ...
    with op.batch_alter_table("metric_value", schema=None) as batch_op:
        batch_op.create_index(batch_op.f("ix_metric_value_diagnostic"), ["diagnostic"], unique=False)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("metric_value", schema=None) as batch_op:
        ...
        batch_op.drop_index(batch_op.f("ix_metric_value_diagnostic"))

    op.drop_table("metric_value")
    op.drop_table("execution_result_dataset")
    with op.batch_alter_table("execution_output", schema=None) as batch_op:
    ...
    with op.batch_alter_table("execution_group", schema=None) as batch_op:
        batch_op.drop_index(batch_op.f("ix_execution_group_dataset_key"))
